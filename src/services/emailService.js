...
function buildAccountDeletionOtpEmailHtml({ otpCode, expiresInMinutes }) {
  const safeOtpCode = escapeHtml(otpCode);
  const safeExpiry = escapeHtml(expiresInMinutes);
  const brandName = escapeHtml(env.mailtrap.senderName || 'Pulse Wise');
  const currentYear = new Date().getFullYear();
  ...
  <html lang="id">
    <head>
      ...
      <meta name="color-scheme" content="light only" />
      <meta name="supported-color-schemes" content="light only" />
      <title>${brandName} - Konfirmasi Hapus Akun</title>
    </head>
    <body style="margin: 0; padding: 0; background-color: #FDF7F8; font-family: -apple-system, BlinkMacSystemFont, 'Inter', 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; color: #3B4A54;">
        ...
        <tr>
          <td align="center">
            <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width: 480px; background-color: #FFFFFF; border-radius: 24px; overflow: hidden; box-shadow: 0 10px 40px rgba(225, 61, 90, 0.08);">
              <tr>
                <td style="background-color: #F99B9F; padding: 32px; text-align: center;">
                  <img src="${logoUrl}" alt="${brandName} Logo" width="200" style="display: inline-block; border: 0; max-width: 100%; height: auto;">
                </td>
              </tr>
              <tr>
                <td style="padding: 40px 32px 32px; background-color: #FFFFFF;">
                  ...
                  <p style="margin: 0 0 28px 0; font-size: 15px; line-height: 1.6; color: #4A5568;">
                    Halo,<br><br>
                    Kami menerima permintaan untuk <strong>menghapus akun Pulse Wise Anda secara permanen</strong>. Tindakan ini tidak dapat dibatalkan dan seluruh data akun akan dihapus.
                  </p>
                  ...
                  <p style="margin: 0 0 24px 0; font-size: 14px; line-height: 1.6; color: #4A5568;">
                    Kode ini hanya berlaku selama <strong>${safeExpiry} menit</strong>. Jika Anda tidak meminta penghapusan akun, segera abaikan email ini dan jangan bagikan kodenya.
                  </p>
                </td>
              </tr>
              <tr>
...
async function sendAccountDeletionOtpEmail({ toEmail, otpCode, expiresInMinutes }) {
  const client = getClient();
  if (!client) {
    const error = new Error('Mailtrap belum dikonfigurasi');
    error.statusCode = 501;
  ...
  await client.send({
    from: sender,
    to: [{ email: toEmail }],
    subject: 'Pulse Wise - Kode OTP Hapus Akun',
    text: `Kode OTP hapus akun Anda adalah ${otpCode}. Berlaku selama ${expiresInMinutes} menit.`,
    html: buildAccountDeletionOtpEmailHtml({
      otpCode,
      expiresInMinutes,
    }),
    category: 'Account Deletion OTP',
  });
}
